...
import type { Professional, DailyAvailability, TimeSlot, ServiceProviderProfile } from '../../types';
import { Clock, Plus, Trash2, Save, Loader2, AlertTriangle } from 'lucide-react';
...
export const AvailabilityManagement = () => {
    ...
    useEffect(() => {
        ...
        if (professionals.length > 0 && !professionals.some(p => p.id === selectedProfId)) {
    ...
    useEffect(() => {
        ...
        if (selectedProfessional) {
            ...
            const fullWeekAvailability = weekDays.map(dayName => {
                ...
                return selectedProfessional.availability?.find(a => a.dayOfWeek === dayName) 
    ...
    if (professionals.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-64 text-gray-500 bg-black/20 rounded-2xl p-8">
                ...
                <p className='text-center mt-2'>Cadastre um profissional na seção "Meus Profissionais" para gerenciar a disponibilidade.</p>
            </div>
        );
    }
    ...
    const handleIsAvailableChange = (day: DailyAvailability['dayOfWeek'], isAvailable: boolean) => {
        setAvailability(prev => prev.map(d => d.dayOfWeek === day ? { 
            ...d, 
            isAvailable, 
            slots: isAvailable && d.slots.length === 0 ? [{ start: '09:00', end: '18:00' }] : d.slots 
        } : d));
    };
    ...
    const handleSave = async () => {
        ...
        const professionalToUpdate = professionals.find(p => p.id === selectedProfId);
            ...
            {selectedProfId && (
                <div className="space-y-6">
                    {availability.map(day => (
                        <div key={day.dayOfWeek} className="bg-gray-800/70 p-4 rounded-xl border border-gray-700 transition hover:border-amber-500/50">
                                    ...
                                    {day.slots.map((slot, i) => (
                                        <div key={i} className="flex flex-wrap items-center gap-3">
                                            <div className='flex items-center gap-1'>
                                                ...
                                                <input type="time" value={slot.start} onChange={(e) => handleSlotChange(day.dayOfWeek, i, 'start', e.target.value)} className="bg-gray-900 p-2 rounded-md border border-gray-600 focus:ring-1 focus:ring-amber-500 focus:border-amber-500 transition" />
                                            </div>
                                            <div className='flex items-center gap-1'>
                                                <span className="text-gray-400 text-sm">Até:</span>
                                                <input type="time" value={slot.end} onChange={(e) => handleSlotChange(day.dayOfWeek, i, 'end', e.target.value)} className="bg-gray-900 p-2 rounded-md border border-gray-600 focus:ring-1 focus:ring-amber-500 focus:border-amber-500 transition" />
                                            </div>
                                            <button onClick={() => handleRemoveSlot(day.dayOfWeek, i)} className="text-gray-500 hover:text-red-400 p-2 rounded-full hover:bg-red-900/50 transition" title="Remover Intervalo"><Trash2 size={18} /></button>
                                        </div>
                                    ))}
                                    <button onClick={() => handleAddSlot(day.dayOfWeek)} className="flex items-center gap-2 text-sm text-amber-500 hover:text-yellow-300 mt-2 p-1 rounded-md transition"><Plus size={16} /> Adicionar intervalo</button>
                                </div>
                            )}
                        </div>
                    ))}
                    <div className="flex justify-end pt-4">
                        <button onClick={handleSave} disabled={isSaving || !selectedProfId} className="bg-amber-500 text-black font-semibold px-6 py-3 rounded-lg hover:bg-amber-600 transition flex items-center gap-2 disabled:bg-gray-500 disabled:cursor-not-allowed">
