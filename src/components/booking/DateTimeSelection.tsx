...
export const DateTimeSelection = () => {
  ...
  // Inicia false para evitar spinner se não houver data, o useEffect cuidará disso
  ...
  // ✅ REFATORADO: Renomeado para 'calculateAvailableSlots'.
  // Esta função agora apenas retorna os dados (Promise<string[]>), SEM alterar o estado (setState).
  // Isso remove os efeitos colaterais de dentro da função de cálculo.
  ...
  // ✅ CORREÇÃO DO BUG: useEffect controla a "validade" da requisição
  useEffect(() => {
    let isActive = true; // Flag para rastrear se o componente ou a data mudou

    const fetchSlots = async () => {
      if (date instanceof Date) {
        setIsLoadingTimes(true);
        setAvailableTimes([]); // Limpa visualmente ao iniciar nova busca

        try {
          const slots = await calculateAvailableSlots(date);
          // Só atualiza o estado se esta execução ainda for a "ativa"
          if (isActive) {
    ...
    // Função de limpeza: marca esta execução como inativa se o date mudar ou componente desmontar
    return () => {
      isActive = false;
    };
