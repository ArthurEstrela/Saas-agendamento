...
const HistoryCard = ({ appt }: { appt: EnrichedProviderAppointment }) => (
    <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p className="text-sm text-gray-400">{appt.services.map(s => s.name).join(', ')}</p>
            <p className="text-xs text-gray-500">com {appt.professionalName}</p>
        </div>
        <div className="text-right">
            <p className="text-sm font-semibold text-gray-300">{format(appt.startTime, "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}</p>
            {appt.status === 'completed' ? (
                <span className="flex items-center justify-end gap-2 text-green-400 text-sm mt-1"><CheckCircle size={16}/> Concluído</span>
            ) : (
                <span className="flex items-center justify-end gap-2 text-red-400 text-sm mt-1"><XCircle size={16}/> Cancelado</span>
            )}
        </div>
    </div>
);
...
export const HistoryTab = ({ appointments }: { appointments: EnrichedProviderAppointment[] }) => {
        ...
        <div>
            ...
            <div className="space-y-4">
                {filteredAppointments.length > 0 ? (
                    filteredAppointments.map(appt => <HistoryCard key={appt.id} appt={appt} />)
                ) : (
                    <p className="text-center text-gray-500 mt-16">Nenhum agendamento encontrado no histórico.</p>
                )}
            </div>
        </div>
    );
