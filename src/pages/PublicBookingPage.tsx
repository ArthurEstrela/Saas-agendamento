...
const PublicBookingPage = () => {
  ...
  const handleBack = () => {
    if (user) {
      // Se tem usuário logado, volta para o contexto do Dashboard
      navigate("/dashboard/explore");
    } else {
      // Se é visitante, volta para a busca pública (não pede login)
      navigate("/explore");
    }
  };
    ...
    <div className="min-h-screen bg-[#09090b] text-gray-100 pb-20 relative overflow-x-hidden selection:bg-primary/30 font-sans">
      ...
      {provider && (
        <Helmet>
          ...
          <meta
            name="description"
            content={`Reserve seu horário na ${provider.businessName}. Confira nossos serviços e avaliações.`}
          />
          <meta
            property="og:title"
            content={`Agendar na ${provider.businessName} | Stylo`}
          />
          <meta
            property="og:description"
            content="Agendamento online rápido e fácil."
          />
          <meta
            property="og:image"
            content={provider.logoUrl || "/stylo-logo.png"}
          />
          <meta
            property="og:url"
            content={`https://stylo.app.br/schedule/${provider.publicProfileSlug}`}
          />
        </Helmet>
      )}
      ...
      <div className="max-w-5xl mx-auto md:p-6 p-0 relative z-10">
        ...
        <header className="relative h-64 md:h-80 md:rounded-3xl overflow-hidden bg-zinc-900 shadow-2xl group border-b md:border border-white/5">
          ...
          <Button
            ...
            size="icon"
            onClick={handleBack}
            className="absolute top-4 left-4 z-30 bg-black/40 text-white rounded-full border border-white/10 active:scale-95 transition-transform hover:bg-black/60"
          >
            <ArrowLeft size={20} />
          </Button>
