...
import { ServiceCompletionModal } from './ServiceCompletionModal'; // Importando o novo modal

interface TimelineEventProps {
  appointment: EnrichedProviderAppointment;
  onSelect: (appt: EnrichedProviderAppointment) => void;
  onUpdateStatus: (id: string, status: 'scheduled' | 'cancelled' | 'completed' | 'no-show', finalPrice?: number) => void;
}
...
const TimelineEventCard = ({ appointment, onSelect, onUpdateStatus }: TimelineEventProps) => {
  ...
  const isPastAppointment = isPast(appointment.endTime);
  const isActionable = isPastAppointment && appointment.status === 'scheduled';

  const cardHeight = Math.max(appointment.totalDuration, 60); 
  const topPosition = (appointment.startTime.getHours() * 60) + appointment.startTime.getMinutes();

  const [isModalOpen, setIsModalOpen] = useState(false);

  const handleCompleteService = (finalPrice: number) => {
    onUpdateStatus(appointment.id, 'completed', finalPrice);
  ...
  return (
    <>
      <motion.div
        ...
        layout
        style={{
          top: `${(topPosition / 1440) * 100 * 24}px`, 
          height: `${(cardHeight / 1440) * 100 * 24}px`,
        }}
        className={`absolute left-16 right-0 rounded-lg p-3 text-white flex flex-col justify-start transition-all duration-300 ease-in-out
          ${isPending
      ...
      >
        ...
        {isActionable && (
          <div className="mt-auto pt-2 flex gap-2">
            <button
              onClick={(e) => { e.stopPropagation(); onUpdateStatus(appointment.id, 'no-show'); }}
              className="text-xs bg-gray-600/50 text-gray-300 w-full py-2 rounded-lg hover:bg-gray-600/80 flex items-center justify-center gap-1"
            >
              <XCircle size={14} /> Não Compareceu
            </button>
            <button
              onClick={(e) => { e.stopPropagation(); setIsModalOpen(true); }}
              className="text-xs bg-green-500/20 text-green-400 w-full py-2 rounded-lg hover:bg-green-500/40 flex items-center justify-center gap-1"
            >
      ...
      <ServiceCompletionModal
        ...
        isLoading={false} // Você pode conectar a uma store se a ação for assíncrona
      />
    </>
  );
};
