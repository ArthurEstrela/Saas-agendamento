...
const convertAppointmentTimestamps = (data: Record<string, unknown>): Record<string, unknown> => {
 ...
 */
export const getAppointmentsByProviderId = async (providerId: string): Promise<Appointment[]> => {
    const appointmentsCollection = collection(db, 'appointments');
    const q = query(
        appointmentsCollection,
...
export const updateAppointmentStatus = async (
  ...
  const updateData: { status: Appointment['status'], rejectionReason?: string } = { status };
  if (rejectionReason) {
...
export const getAppointmentsForProfessionalOnDate = async (professionalId: string, date: Date): Promise<Appointment[]> => {
  ...
  return querySnapshot.docs.map(doc => {
      // Reutilize sua função de conversão de timestamp se necessário
      const data = doc.data();
      if (data['startTime'] instanceof Timestamp) {
