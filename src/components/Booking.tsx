...
const Booking = ({ professional: establishment, onBack }: BookingProps) => {
  ...
  return (
    ...
    <div className="bg-gray-900/80 p-6 sm:p-8 rounded-2xl w-full max-w-3xl border border-[#daa520]/30 shadow-2xl shadow-[#daa520]/10">
            ...
            return (
              <React.Fragment key={step.step}>
                <div className="flex flex-col items-center text-center">
                  <div
                    className={`h-10 w-10 rounded-full flex items-center justify-center font-bold border-2 transition-all duration-300 ${
                ...
                {index < bookingSteps.length - 1 && (
                  <div
                    className={`flex-1 h-1 mx-2 ${
                      currentStep > index + 1 ? "bg-[#daa520]" : "bg-gray-700"
                    }`}
                  ></div>
                )}
              </React.Fragment>
      ...
      <div className="mt-8 min-h-[450px]">{renderStepContent()}</div>
