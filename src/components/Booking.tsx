...
const Booking = ({ professional: establishment, onBack }: BookingProps) => {
  ...
  useEffect(() => {
    const generateTimeSlots = (
      ...
      workIntervals.forEach((workInterval) => {
        ...
        const [startHour, startMinute] = workInterval.start
          .split(":")
          .map(Number);
        ...
        while (currentTime < workEndTime) {
          ...
          if (isToday && slotStartTime < now) {
            ...
            const isInBreak = breakIntervals.some((breakInterval) => {
              ...
              const [breakStartHour, breakStartMinute] = breakInterval.start
                .split(":")
                .map(Number);
              ...
              const [breakEndHour, breakEndMinute] = breakInterval.end
                .split(":")
                .map(Number);
              breakEndTimeObj.setHours(breakEndHour, breakEndMinute, 0, 0);
              return (
                slotStartTime < breakEndTimeObj &&
                slotEndTime > breakStartTimeObj
              );
            });
            if (isInBreak) status = "break";

            const isOverlapping = bookedIntervals.some(
              (booked) =>
    ...
    const fetchAvailableTimes = async () => {
      ...
      try {
        const dayKey = (selectedDate as Date)
        ...
        for (const doc of querySnapshot.docs) {
          ...
          const durationOfBooking = appointment.duration;

          if (durationOfBooking > 0 && appointment.startTime) {
            const [hour, minute] = appointment.startTime.split(":").map(Number);
            const startDate = new Date(selectedDate as Date);
            startDate.setHours(hour, minute, 0, 0);
            const endDate = new Date(
              startDate.getTime() + durationOfBooking * 60000
            );
        ...
        if (finalSlots.filter((s) => s.status === "available").length === 0) {
          setAvailabilityMessage(
            "Não há horários disponíveis para esta seleção."
          );
        }
      } catch (error) {
    ...
    if (
      ...
      if (pendingBookingRaw) {
        ...
        try {
          ...
          console.error(
            "Erro ao processar dados de agendamento pendente:",
            error
          );
        ...
        if (
          pendingBooking &&
          pendingBooking.serviceProviderId === establishment.uid
        ) {
          localStorage.removeItem("pendingBooking");

          const professional = (
            establishment.professionals as ProfessionalWithAvailability[]
          )?.find((p) => p.id === pendingBooking.professionalId);
          const services = pendingBooking.serviceIds
          ...
          if (
            professional &&
            services.length === pendingBooking.serviceIds.length &&
            pendingBooking.time
          ) {
            showToast("Finalizando seu agendamento...", "info");
            ...
            showToast(
              "Não foi possível restaurar os detalhes do seu agendamento. Por favor, tente novamente.",
              "warning"
            );
          }
        }
  ...
  const handleNextStep = () => {
    ...
    if (
      currentStep === 3 &&
      (!selectedDate || Array.isArray(selectedDate) || !selectedTime)
    ) {
                ...
                professionalsForSelectedServices.map((prof) => (
                  ...
                  >
                    <img
                      src={
                        prof.photoURL ||
                        "https://placehold.co/150x150/1F2937/4B5563?text=Prof"
                      }
                      alt={prof.name}
        ...
        return (
          <div className="animate-fade-in-up">
            ...
            {selectedProfessional ? (
              ...
              <p className="text-gray-500 text-center py-8">
                Por favor, selecione um profissional primeiro.
              </p>
            )}
          </div>
        );
        ...
        return (
          <div className="animate-fade-in-up">
            ...
            selectedTime ? (
              <div className="bg-gray-800 p-6 rounded-xl border border-[#daa520] animate-fade-in-down">
                ...
                <div className="space-y-3 text-gray-300">
                  ...
                  <div className="flex justify-between">
                    <span>Data:</span>
                    <span className="font-semibold text-white">
                      {(selectedDate as Date).toLocaleDateString("pt-BR")}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Horário:</span>
              ...
              <p className="text-gray-500 text-center py-8">
                Por favor, complete as etapas anteriores para confirmar seu
                agendamento.
              </p>
            )}
          </div>
    ...
    <>
      <LoginModal
        ...
        onClose={() => {
          ...
          setIsAwaitingLogin(false); // Cancela a espera se o usuário fechar o modal
        }}
        onLoginSuccess={() => {
