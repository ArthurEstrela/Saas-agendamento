...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  signInWithGoogle: async (role: "CLIENT" | "SERVICE_PROVIDER") => {
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          if (role === "CLIENT") {
            ...
            const regResponse = await api.post<UserProfile>(
      ...
      set({
        error: extractErrorMessage(error, "Erro na autenticação com o Google."),
        loading: false,
      });
      throw error;
    }
  ...
  logout: async () => {
    ...
    try {
      ...
      set({
        error: extractErrorMessage(error, "Erro ao sair da conta."),
        loading: false,
      });
    }
  },
      ...
      if (firebaseUser) {
        try {
          ...
          if (isAxiosError(error) && error.response?.status === 404) {
            ...
            console.error(
              "Sessão inválida ou expirada no servidor. Forçando logout.",
              error,
            );
